# Piano d'Azione Aggiornato: JsonPages Platform (v3.2)

> **Stato al termine del Giorno 1**: L'infrastruttura di base è solida. Il Frontend (Angular/Vite) comunica con il Backend (NestJS) tramite Proxy. I file di configurazione (`theme.json`, ecc.) vengono serviti correttamente, ma attualmente usiamo un controller generico (`AppController`) che va rifattorizzato.
...
1. **Refactoring Backend:** Creare `ConfigController` e `ContentController` separati.
2. **Frontend Startup:** Implementare `APP_INITIALIZER` per il tema.
3. **Data Binding:** Visualizzare la lista degli atleti (`items.json`) usando `GridListComponent` e `ContentController`.